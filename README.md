...
<p align="center">
  ...
  <img src="docs/screenshots/stats.png" width="150" alt="Статистика прослушиваний">
...
- **Статистика прослушиваний.** Бот следит за плеером Яндекс Музыки — в приложении, на сайте, на колонке — и считает каждое прослушивание, включая повторы, и реальное время: любимые исполнители, треки, альбомы и жанры, откуда играла музыка, новые открытия, серии дней с музыкой и сравнение с прошлым периодом. Итоги недели и месяца приходят в чат сами, итоги дня — по желанию; треки и исполнители в итогах кликабельны. Включается по кнопке, `/stats` или в «Медиатеке».
...
Включается самим человеком: `/stats` → «📊 Включить статистику» или «Моя статистика» в «Медиатеке». Выключить
и удалить собранное можно там же в любой момент.

- **Каждое прослушивание, включая повторы.** Бот подключается к Ynison — через него приложения Яндекс Музыки
  (телефон, компьютер, сайт, колонки) синхронизируют плеер — как невидимое устройство: не плеер и не пульт, ничего
  не переключает. Он видит каждую смену трека, паузу, перемотку и повтор и засчитывает прослушивание, как Last.fm:
  трек слушали хотя бы половину или 4 минуты (что меньше). Трек на повторе 10 раз — 10 прослушиваний, время —
  сколько реально слушали. Пропущенные треки не считаются.
- **Плеер «Медиатеки»** сообщает о своих прослушиваниях сам по тому же правилу (Яндекс о них не знает).
- **История Яндекса — запасной источник.** Раз в час бот забирает историю аккаунта и добирает из неё то, чего не
  видел вживую: бот был выключен, трек играл на устройстве без Ynison. Там нет времени и повторов, поэтому такое
  прослушивание — одно за день, а время — по длительности трека (в приложении это помечено, в итогах время с «≈»).
- **Что видно:** прослушивания и время, дни с музыкой и серия дней подряд, график по дням (за год — по месяцам),
  топ исполнителей, треков (с числом повторов) и альбомов, жанры, откуда играла музыка (Моя волна, плейлисты,
  альбомы, «Медиатека»), новые треки и исполнители, изменение к прошлому периоду. Периоды — день, неделя, месяц и
  год, можно листать назад.
- **Всё кликабельно.** В приложении исполнитель и альбом открываются своей страницей, трек сразу играет.
  В итогах в чате треки и исполнители — ссылки на бота: нажали на трек — он пришёл в чат, на исполнителя —
  открылись его треки (`t.me/бот?start=t…` / `ar…` / `al…`).
- **Итоги в чат** (время московское): неделя — в воскресенье в 21:00, месяц — 1-го числа в 10:00 за прошедший месяц,
  день — в 22:00, если включить. Настраиваются в `/stats` → «⚙️ Итоги в чат» или внизу экрана статистики. Если бот
  был выключен в нужный момент, итоги придут, когда он запустится (в пределах нескольких часов), и не придут дважды.

Что стоит знать:

- **Статистика копится с момента включения.** Яндекс хранит историю недолго, так что за прошлые месяцы бот знает
  только то, что ещё успел отдать Яндекс при первой выгрузке. Первый неполный период помечается «статистика только
  начала собираться».
- **Ynison и история — неофициальные API.** Если Яндекс изменит протокол плеера или формат истории и что-то
  перестанет работать у нескольких человек, владелец получит сообщение, а в админ-панели появится предупреждение.
  Без Ynison статистика продолжит собираться по истории (без повторов); точный подсчёт можно выключить в
  админ-панели → «Режимы». Проверить всё на своём аккаунте: админ-панель → «Обзор» → «Мой плеер вживую (проверка)»
  (что бот видит в плеере прямо сейчас и что засчитал) и «Моя история Яндекса (проверка)».
- **Нагрузка.** На каждого, у кого включена статистика, бот держит одно websocket-соединение с Яндексом (с пингами
  раз в 20–30 секунд). Для сотен человек это копейки по памяти и трафику.
...
- **в «Медиатеке»** — щит 🛡 рядом с шестерёнкой (и пункт «Админ-панель» в настройках):
  ...
  - **Режимы:** техработы со своим текстом (бот отвечает только админам), закрытая регистрация (новые не допускаются,
    старые пользуются как раньше), точный подсчёт прослушиваний (Ynison) вкл/выкл, лимиты скачиваний и загрузок
    на человека в сутки;
...
- **SQL-инъекции.** Все значения уходят в базу только параметрами (`?`), поиск людей экранирует `%` и `_`.
  Тест `tests/test_sql_safety.py` проверяет исходники: запрос, собранный из данных, не пройдёт. Второй рубеж —
  после запуска соединение с базой не может подключить другой файл (ATTACH), поменять схему и настройки или
  загрузить расширение, так что даже внедрённый SQL не выйдет за пределы обычного чтения и записи строк.
...
- **Статистика** («Моя статистика» в «Медиатеке»): периоды день / неделя / месяц / год с листанием назад, плитки с итогами, график по дням, топы исполнителей, треков (с числом повторов), альбомов и жанров, источники, новые открытия и настройка итогов в чат. Исполнители, альбомы и открытия открываются своими страницами, треки играют сразу.
...
Если с сервера бота Яндекс не открывается или открывается через раз (бот пишет «Яндекс Музыка сейчас недоступна»,
всё работает медленно), весь трафик к Яндексу — API, скачивание, загрузка, плеер, вход и точный подсчёт — можно
пустить через прокси. Telegram при этом ходит как раньше. Проверить связь тем же путём, что и бот:

```bash
docker compose exec bot python -m bot.diag_net
```

**Вариант А: второй сервер по SSH (рекомендуется).** Подойдёт любой недорогой сервер, откуда Яндекс открывается
(проверьте на нём: `curl -4 -m 10 -sS -o /dev/null -w "%{http_code}\n" https://api.music.yandex.net/account/status`
должен ответить `401`). Ставить на него ничего не нужно — бот поднимет SOCKS-прокси через обычный SSH.

1. На сервере бота создайте ключ и добавьте его на второй сервер (спросит пароль второго сервера один раз):
   ```bash
   cd ~/yamusicbot && mkdir -p ssh && ssh-keygen -t ed25519 -N "" -f ssh/id_ed25519
   ssh-copy-id -i ssh/id_ed25519.pub root@IP_ВТОРОГО_СЕРВЕРА
   ```
2. Впишите в `.env` (если в `COMPOSE_PROFILES` уже что-то есть — добавьте через запятую, например `tunnel,yandex-tunnel`):
   ```env
   YANDEX_SSH=root@IP_ВТОРОГО_СЕРВЕРА
   COMPOSE_PROFILES=yandex-tunnel
   ```
   SSH не на 22-м порту — ещё `YANDEX_SSH_PORT=2222`.
3. Перезапустите: `docker compose up -d --build --force-recreate`. Проверка: `docker compose logs --tail 20 yandex-tunnel`
   (строка «Туннель до …») и `docker compose exec bot python -m bot.diag_net` (должно быть «через прокси» и ✅).

Туннель — контейнер `yandex-tunnel` с autossh: сам переподключается, прокси доступен только контейнерам этого
docker compose (порт наружу не открыт), ключ лежит в `./ssh` и в образ бота не попадает. Для надёжности вместо root
можно завести на втором сервере отдельного пользователя без пароля и положить ключ ему.

**Вариант Б: готовый прокси.** Если есть SOCKS5 или HTTP-прокси, откуда Яндекс работает:
```env
YANDEX_PROXY=socks5://логин:пароль@адрес:порт
```
(или `http://логин:пароль@адрес:порт`). Логин и пароль в логах и админ-панели не показываются.
...
| `/stats` | статистика прослушиваний: включить, итоги за день, неделю, месяц и год (треки и исполнители — ссылки), итоги в чат |
...
- **Загрузка через неофициальный API.** Ответы адреса загрузки, по которым видно, что Яндекс его изменил
  ...
  снова работает.
- **Сеть до Яндекса.** Библиотека yandex-music открывает новое соединение на каждый запрос и ждёт 5 секунд — на
  плохом маршруте (часть путей до Яндекса у хостера сломана) это почти всегда таймаут. Поэтому все соединения
  с Яндексом идут через `bot/net.py`: соединение устанавливается с нескольких попыток (новое уходит другим путём),
  удачное переиспользуется, на ответ API — до 60 секунд (`YandexRequest` в `bot/ym.py`). Когда Яндекс совсем
  недоступен, бот говорит об этом прямо («Яндекс Музыка сейчас недоступна»), а не «что-то пошло не так».
- **Статистика прослушиваний.** Три источника: Ynison (`bot/ynison.py` — протокол, `bot/live.py` — прослушивания
  из кадров плеера и соединения), плеер «Медиатеки» (`POST /api/stats/played`) и история Яндекса
  (`bot/listening.py`). Живые прослушивания — таблица `plays` (каждое — строка), история — `listens` («трек в день»),
  а `live_seen` помнит, какие треки бот видел вживую, включая пропущенные. Единый набор собирает `Storage._PLAYS`:
  живые + история там, где вживую ничего не видели. Итоги считаются SQL-запросом на лету, агрегатов нет.
  Сбои разделены так же, как у загрузки: отозванный вход и нет сети — про человека, непонятный ответ — про API
  (`admin.history_health`, `admin.live_health`, те же пороги). Разбор кадров Ynison терпим к snake_case и camelCase,
  числам строками и перечислениям числами; токен уходит только на хосты Яндекса.
...
bot/
  ...
  net.py            соединения с Яндексом: прокси (YANDEX_PROXY), повтор установки, keep-alive
  diag_net.py       проверка связи с Яндексом тем же путём, что и бот
  ...
  listening.py      статистика прослушиваний: история Яндекса, подсчёт, итоги дня/недели/месяца в чат
  live.py           прослушивания в реальном времени: кадры плеера → прослушивания, соединения с Ynison
  ynison.py         протокол Ynison (синхронизация плеера Яндекс Музыки): подключение и разбор кадров
