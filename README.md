...
**Вариант А: свой домен (для постоянной работы).** Направьте A-запись домена на сервер, откройте порты 80 и 443 и впишите в `.env`:
```bash
DOMAIN=music.example.com
WEBAPP_URL=https://music.example.com
```
Затем запустите вместе со встроенным Caddy, он сам получит сертификат Let's Encrypt:
```bash
