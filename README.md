...
- **Админ-панель для владельца.** Статистика, пользователи с поиском, блокировки, рассылка, техработы, закрытая регистрация, дневные лимиты, журнал действий, ошибки по коду и бэкап базы — в «Медиатеке» и командой `/admin`. Видна только Telegram ID из `ADMIN_IDS`.
...
После этого у вас (и только у вас) появятся:
...
- **в чате** — `/admin` (сводка и кнопки: техработы, регистрация, рассылка, бэкап, ошибки, журнал),
  `/user ID`, `/ban ID причина`, `/unban ID`, `/broadcast`. Остальным эти команды не видны и не работают.

Как защищена:

- Админов задаёт только `ADMIN_IDS` в `.env` — из самой панели добавить админа нельзя.
...
- Для всех остальных админ-API не существует (ответ 404). Попытка попадает в журнал, а вам приходит сообщение 🚨.
- Бэкап базы (в нём входы пользователей) приходит только вам в личный чат с ботом — скачать его по ссылке нельзя.
...
- Входы пользователей хранятся в `data/bot.db` без шифрования (права 600). Берегите сервер: вход по SSH-ключу,
  обновления системы, `chmod 600 .env`.
...
bot/
  ...
  errors.py         тексты ошибок для чата: без адресов запросов (в адресах файлов Telegram — токен бота)
  admin.py          админка: доступ, блокировки, техработы, лимиты, статистика, рассылка, журнал, бэкап
