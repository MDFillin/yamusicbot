...
- **Статистика прослушиваний.** Бот каждый час забирает историю из Яндекс Музыки — всё, что вы слушали в приложении, на сайте и колонках, — и показывает, сколько и что вы слушаете: любимые исполнители, треки, альбомы и жанры, откуда играла музыка, новые открытия, серии дней с музыкой и сравнение с прошлым периодом. Итоги недели и месяца приходят в чат сами, итоги дня — по желанию. Включается по кнопке, `/stats` или в «Медиатеке».
...
- **Откуда данные.** Раз в час бот забирает у Яндекса историю прослушивания аккаунта — ту же, что в разделе «История»
  приложения. Она общая для всех устройств, поэтому считается музыка из приложения, с сайта и колонок, а не только
  из бота.
- **Что видно:** прослушивания и примерное время, дни с музыкой и серия дней подряд, график по дням (за год — по
  месяцам), топ исполнителей, треков и альбомов, жанры, откуда играла музыка (Моя волна, плейлисты, альбомы),
  новые треки и исполнители, изменение к прошлому периоду. Периоды — день, неделя, месяц и год, можно листать назад.
...
Ограничения — их стоит понимать, прежде чем сравнивать с «Итогами года» Яндекса:

- **История Яндекса знает день, но не время и не число повторов.** Поэтому одно прослушивание — это трек в конкретный
  день: трек, прослушанный 5 раз за вечер, засчитается один раз. Время — оценка по длительности треков.
- **Статистика копится с момента включения.** Яндекс хранит историю недолго, так что за прошлые месяцы бот знает
  ...
  начала собираться».
- **Это неофициальный API.** Если Яндекс изменит формат истории и она перестанет разбираться у нескольких человек,
  владелец получит сообщение, а в админ-панели появится предупреждение. Сверить подсчёт со своими данными можно в
  админ-панели → «Обзор» → «Моя история Яндекса (проверка)»: там дни с числом треков и ответ Яндекса как есть.
...
- **Статистика** («Моя статистика» в «Медиатеке»): периоды день / неделя / месяц / год с листанием назад, плитки с итогами, график по дням, топы исполнителей, треков, альбомов и жанров, источники, новые открытия и настройка итогов в чат.
...
| `/stats` | статистика прослушиваний: включить, итоги за день, неделю, месяц и год, итоги в чат |
...
- **Загрузка через неофициальный API.** Ответы адреса загрузки, по которым видно, что Яндекс его изменил
  ...
  снова работает.
- **История прослушивания** (`bot/listening.py`) устроена так же: отозванный вход и нет сети — это про человека,
  а ответ, который не получается разобрать, — про API (`admin.history_health`, те же пороги). В базе хранится
  «трек в день» (`listens`), данные треков и названия плейлистов кэшируются общими на всех (`track_meta`, `contexts`),
  а сами итоги считаются SQL-запросом на лету: отдельных агрегатов нет, пересчитывать ничего не нужно.
...
bot/
  ...
  listening.py      статистика прослушиваний: сбор истории Яндекса, подсчёт, итоги дня/недели/месяца в чат
