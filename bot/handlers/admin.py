...
def panel(admin: Admin, viewer: int) -> tuple[str, InlineKeyboardMarkup]:
    ...
    text = (
        ...
        f"📊 Статистику прослушиваний включили: {len(admin.store.stats_users())}"
        f"{' · ❌ сбор истории сломался' if admin.history_health.json()['broken'] else ''}\n"
        f"🎯 Точный подсчёт: <b>{'включён' if s.live_tracking else 'выключен'}</b>"
        f"{' · ❌ не работает' if admin.live_health.json()['broken'] else ''}\n\n"
