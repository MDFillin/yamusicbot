...
Я буду раз в час забирать вашу историю из Яндекс Музыки — из приложения, с сайта и колонок — и считать:
• сколько и что вы слушаете, любимых исполнителей, треки и жанры;
• новые открытия и серии дней с музыкой;
• сравнение с прошлой неделей и месяцем.

Итоги недели придут в воскресенье вечером, месяца — 1-го числа, дня — по желанию.

<i>Яндекс хранит историю недолго, поэтому статистика копится с момента включения. В истории нет повторов \
за день, так что прослушивание — это трек в конкретный день, а время — оценка. Выключить и удалить \
статистику можно в любой момент.</i>"""
...
async def show_stats(target: Message, user_id: int, listening: Listening, kind: str = "week", offset: int = 0,
    ...
    await _show(target, listening.report_text(user_id, p), listening.report_markup(p, offset), edit)
